...
export const mockApiAdapter = {
  ...
  async checkinInterview(slotId: string, applicantPhoneOrId: string, operatorId: string): Promise<User> {
    await delay();
    const applicant = db.users.find(u => u.phone === applicantPhoneOrId || u.id === applicantPhoneOrId);
    if (!applicant) throw new NotFoundError("报名用户不存在");

    const app = db.applications.find(a => a.userId === applicant.id);
    if (!app) throw new NotFoundError("未找到报名材料");

    const slot = db.interviewSlots.find(s => s.id === slotId);
    if (!slot) throw new NotFoundError("场次不存在");
  ...
  async correctAttendance(attendanceId: string, update: {
    ...
    if (update.checkInTime) record.checkInTime = update.checkInTime;
    if (update.checkOutTime) record.checkOutTime = update.checkOutTime;
